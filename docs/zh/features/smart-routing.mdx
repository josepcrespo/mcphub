...
| `gemini-embedding-001` | 3072 | HNSW (halfvec) | Google Gemini — 免费额度（100 RPM），兼容 OpenAI 接口 |
...
<Warning>
  **向量维度限制（pgvector 0.7.0+）**
  ...
  对于 `text-embedding-3-large`（3072 维）：
...
# 通过 OpenAI 兼容接口使用 Google Gemini（免费额度：100 RPM）
