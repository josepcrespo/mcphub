...
- Verifies a deterministic tool-set hash stored in embedding metadata. The hash is derived from the current tool list (name, description, and input schema) so equal counts alone do not cause a false skip.

Only when both the exact content IDs and the stored tool-set hash match does MCPHub skip regeneration.
