...
Additionally, a skip-check optimization avoids unnecessary re-generation: before Phase 1, MCPHub may count existing embeddings for the server+model and skip the entire sync when the stored count matches the number of known tools.
